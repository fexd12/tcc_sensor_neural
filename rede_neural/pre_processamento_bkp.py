...
class PreProcessamento(DataLoader):
    def __init__(self, feats: Feats, filename: str, sfreq: int) -> None:
        ...
        temp_epochs = self.pre_process(
            emcp_epochs=False, rereference=False,
        ...
        self.feature_engineer(temp_epochs,
    ...
    def pre_process(self, plot_psd=False, filter_data=True,
        ...
        events = find_events(self.raw, shortest_event=1)
        self.event_id = {str(x[2]): x[2] for idx, x in enumerate(events)}

        # pull event names in order of trigger number

        event_names = [x[0] for x in self.event_id.items()]
        i = 0
    ...
    def feature_engineer(self, epochs: Epochs, frequency_domain=False,
        ...
        # pull event names in order of trigger number
        print(epochs)
        epochs.event_id = self.event_id
        event_names = [x[0] for x in epochs.event_id.items()]
        i = 0
        for key, value in sorted(epochs.event_id.items(), key=lambda x: (x[1], x[0])):
            event_names[i] = key
            i += 1
        ...
        if frequency_domain:
            ...
            # if len(epochs[event]) == 0:
            #     continue

            tfr_temp = tfr_morlet(epochs, freqs=frequencies,
